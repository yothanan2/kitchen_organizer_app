...
final butcherRequestableItemsProvider = StreamProvider.autoDispose<QuerySnapshot>((ref) {
  ...
  return firestore.collection('inventoryItems').where('isButcherItem', isEqualTo: true).snapshots();
});
...
class _ButcherRequisitionScreenState extends ConsumerState<ButcherRequisitionScreen> {
  DateTime _selectedDate = DateTime.now().add(const Duration(days: 1)); // This is the line we discussed changing to DateTime.now() for testing
