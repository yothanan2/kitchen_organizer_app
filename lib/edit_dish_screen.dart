...
// FINAL CORRECTION: Updated to use the correct constructor parameters.
...
class _EditDishScreenState extends State<EditDishScreen> {
  ...
  void initState() {
    ...
    if (_isEditing) {
      ...
      _isActive = dish.isActive;
      _isComponent = dish.isComponent;
      _loadSubCollections();
    } else {
  ...
  Future<void> _loadSubCollections() async {
    ...
    final dishRef = FirebaseFirestore.instance.collection('dishes').doc(widget.dish!.id);
    ...
    final prepTasksSnapshot = await dishRef.collection('prepTasks').orderBy('order').get();
    if (!mounted) return;
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  ...
                  if (selectedInventoryItemId != null && (isQuantifiedValid || isOnHandValid)) {
                    navigator.pop({'inventoryItemId': selectedInventoryItemId, 'quantity': isOnHand ? null : num.tryParse(quantityController.text) ?? 0, 'unitId': isOnHand ? null : selectedUnitId, 'type': isOnHand ? 'on-hand' : 'quantified'});
                  } else {
          ...
          ElevatedButton(
            onPressed: () {
              if (taskController.text.trim().isNotEmpty) {
                navigator.pop({'taskName': taskController.text.trim(), 'linkedDishId': linkedDishId});
              } else {
  ...
  Future<void> _saveDish() async {
    ...
    try {
      ...
      if (_isEditing) {
        dishRef = FirebaseFirestore.instance.collection('dishes').doc(widget.dish!.id);
  ...
  Widget build(BuildContext context) {
    final bool isCreatingFromDishList = !_isEditing && !widget.isCreatingComponent;
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              if (isCreatingFromDishList || (_isEditing && !_isComponent)) ...[
                const SizedBox(height: 16),
